...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box; }
    ...
    .chip,.icon-btn,.mode-toggle,.copy-btn { border:1px solid var(--line); background:#fff; color:#334155; border-radius:6px; height:28px; padding:0 10px; display:inline-flex; align-items:center; justify-content:center; gap:6px; }
...
<body data-source-html="${encodedSource}" data-source-file="${safeFileName}">
  ...
  <script>
    const sourceHtml = new TextDecoder().decode(Uint8Array.from(atob(document.body.dataset.sourceHtml), c => c.charCodeAt(0)));
    ...
    const guides = ["measureTop","measureRight","measureBottom","measureLeft"].map(id => document.getElementById(id));
    ...
    function tagLabel(el) { const tag = el.tagName.toLowerCase(); const id = el.id ? "#" + el.id : ""; const className = [...el.classList].slice(0,2).map(n => "." + n).join(""); const text = compactText(el.innerText || el.textContent || ""); return { tag, id, className, text }; }
    ...
    function grid(items) { return `<div class="primary-grid">${items.map(([k,v]) => `<div class="primary-card"><div class="primary-label">${escapeText(k)}</div><div class="primary-value">${escapeText(v)}</div></div>`).join("")}</div>`; }
    function kv(items) { return `<div class="kv">${items.map(([k,v]) => `<div class="key">${escapeText(k)}</div><div>${escapeText(v)}</div>`).join("")}</div>`; }
    function details(title, html) { return `<details open><summary>${escapeText(title)}</summary><div class="detail-inner">${html}</div></details>`; }

    function assignIds() { let i = 1; getDoc().querySelectorAll("*").forEach(el => { if (isInspectable(el)) { el.dataset.handoffId = String(i++); el.style.cursor = "crosshair"; } }); }
    function buildRecords(root, depth = 0, list = []) { [...root.children].forEach(child => { if (!isInspectable(child)) { buildRecords(child, depth, list); return; } const label = tagLabel(child); list.push({ id: child.dataset.handoffId, depth, label, search: [label.tag,label.id,label.className,label.text].join(" ").toLowerCase() }); buildRecords(child, depth + 1, list); }); return list; }
    function findElement(id) { return getDoc().querySelector(`[data-handoff-id="${id}"]`); }
    function renderLayers() { const q = document.getElementById("layerSearch").value.trim().toLowerCase(); const rows = q ? records.filter(r => r.search.includes(q)) : records; layerTree.innerHTML = rows.map(r => `<div class="tree-node ${r.id === selectedId ? "selected" : ""}" data-id="${r.id}" style="padding-left:${8 + r.depth * 14}px"><span>›</span><span class="tree-label"><span class="tag">${r.label.tag}</span><span class="class-name">${escapeText(r.label.id + r.label.className)}</span> <span class="text-preview">${escapeText(r.label.text)}</span></span></div>`).join(""); }
    ...
    function specsJson() { const els = records.map(r => findElement(r.id)).filter(isInspectable); return { meta:{ format:"html-design-handoff/specs-v1", sourceFile:document.body.dataset.sourceFile, generatedAt:new Date().toISOString(), nodeCount:els.length, viewport:{ width:frame.contentWindow.innerWidth, height:frame.contentWindow.innerHeight } }, layers: els.map(el => { const r = el.getBoundingClientRect(), s = el.ownerDocument.defaultView.getComputedStyle(el), label = tagLabel(el); return { id:el.dataset.handoffId, selector:stableSelector(el), type:label.tag, text:label.text, bounds:{ left:r.left, top:r.top, width:r.width, height:r.height }, textStyle:{ fontFamily:s.fontFamily, fontSize:s.fontSize, fontWeight:s.fontWeight, lineHeight:formatMeasure(resolvedLineHeight(el,s)), color:s.color }, spacing:{ margin:s.margin, padding:s.padding, gap:s.gap }, appearance:{ background:s.backgroundColor, border:s.border, borderRadius:s.borderRadius, boxShadow:s.boxShadow } }; }) }; }
    function renderTokens() { const colors = new Map(), fonts = new Map(); records.map(r => findElement(r.id)).filter(isInspectable).forEach(el => { const s = el.ownerDocument.defaultView.getComputedStyle(el); [s.color,s.backgroundColor,s.borderColor].forEach(v => (String(v).match(/rgba?\([^)]+\)|#[0-9a-fA-F]{3,8}/g) || []).forEach(c => colors.set(c,(colors.get(c)||0)+1))); fonts.set(s.fontSize,(fonts.get(s.fontSize)||0)+1); }); return section("Colors", kv([...colors.entries()].slice(0,24).map(([k,v]) => [k,String(v)]))) + section("Font Sizes", kv([...fonts.entries()].slice(0,24).map(([k,v]) => [k,String(v)]))); }
    ...
    function setup() { assignIds(); records = buildRecords(getDoc().body || getDoc().documentElement); document.getElementById("nodeCount").textContent = String(records.length); document.getElementById("viewportSize").textContent = "画板 " + frame.contentWindow.innerWidth + " × " + frame.contentWindow.innerHeight + "px"; renderLayers(); renderInspect(); getDoc().addEventListener("click", e => { if (mode === "interactive") return; const el = e.target.closest("[data-handoff-id]"); if (el) { e.preventDefault(); e.stopPropagation(); selectElement(el); } }, true); getDoc().addEventListener("mouseover", e => { if (mode === "interactive") return; const el = e.target.closest("[data-handoff-id]"); if (el && el.dataset.handoffId !== selectedId) placeBox(el, hoverBox); }, true); getDoc().addEventListener("mouseout", () => hoverBox.style.display = "none", true); frame.contentWindow.addEventListener("scroll", () => updateOverlay(selectedId ? findElement(selectedId) : null), true); }
    layerTree.addEventListener("click", e => { const row = e.target.closest("[data-id]"); if (!row) return; const el = findElement(row.dataset.id); el?.scrollIntoView({ block:"center", inline:"center" }); selectElement(el); });
    document.getElementById("layerSearch").addEventListener("input", renderLayers);
    document.querySelectorAll(".tab").forEach(btn => btn.addEventListener("click", () => { activeTab = btn.dataset.tab; document.querySelectorAll(".tab").forEach(x => x.classList.toggle("active", x === btn)); renderInspect(); }));
    document.getElementById("copyCssBtn").addEventListener("click", () => navigator.clipboard.writeText(currentCss));
    document.getElementById("copyJsonBtn").addEventListener("click", () => navigator.clipboard.writeText(JSON.stringify(specsJson(), null, 2)));
    document.getElementById("modeBtn").addEventListener("click", e => { mode = mode === "inspect" ? "interactive" : "inspect"; e.target.textContent = mode === "inspect" ? "标注模式" : "交互模式"; e.target.classList.toggle("interactive", mode === "interactive"); });
    document.getElementById("refreshBtn").addEventListener("click", () => { selectedId = null; frame.srcdoc = sourceHtml; });
